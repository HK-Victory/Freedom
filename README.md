...
### 5. 邮件倒计时提醒
- **页面可配置**：「邮件配置 → 定时提醒设置」中可开关、设置**提前提醒天数（如 1/3/7 天）**；**每日执行时间固定为 20:00（北京时间）**，由 Vercel Cron 决定，页面不可修改
- 由 Vercel Cron 每日 20:00（北京时间）触发 `/api/cron/reminders`，仅对剩余天数属于配置范围的任务真正发送
...
- **#2/#3 提醒可配置**：新增「邮件配置 → 定时提醒设置」面板（仅超管），可开关提醒、设置**提前提醒天数（如 1/3/7 天）**；**发送时间固定为每日 20:00（北京时间），由 Vercel Cron 决定**
- 新增 `settings` 表与 `GET/PUT /api/settings/reminder` 接口（超管）
- `vercel.json` 增加 `crons`（每日 20:00 北京时间触发 `/api/cron/reminders`），后端按提前天数过滤后才真正发送邮件（已移除「页面配置小时 == 当前小时」门禁，避免改页面时间后提醒被静默跳过）
